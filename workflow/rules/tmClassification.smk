rule compareTargetsToTms:
    input:
        ...
        targetedSegments=lambda wildcards: CAPDESIGNTOTARGETSGFF[
...
rule getTargetCoverageStats:
    ...
    output:
        "output/statsFiles/"
        + "tmp/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.min{minReadSupport}reads.targetCoverage.stats.tsv",
    shell:
        r"""
uuidTmpOut=$(uuidgen)
...
rule aggTargetCoverageStats:
    ...
    output:
        "output/statsFiles/" + "all.min{minReadSupport}reads.targetCoverage.stats.tsv",
    shell:
        r"""
uuidTmpOut=$(uuidgen)
...
if SIRVpresent:
    ...
    rule getGffCompareSirvStats:
        ...
        output:
            "output/statsFiles/"
            + "tmp/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.{filt}.tmerge.min{minReadSupport}reads.vs.SIRVs.stats.tsv",
        shell:
            r"""
uuidTmpOut=$(uuidgen)
uuid=$(uuidgen)
...
if SIRVpresent:
    ...
    rule getSirvDetectionStats:
        ...
        output:
            "output/statsFiles/"
            + "tmp/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.vs.SIRVs.detection.stats.tsv",
        shell:
            r"""
uuid=$(uuidgen)
uuidTmpOut=$(uuidgen)
...
if SIRVpresent:
    ...
    rule aggSirvDetectionStats:
        ...
        output:
            "output/statsFiles/"
            + "all.tmerge.min{minReadSupport}reads.vs.SIRVs.detection.stats.tsv",
        shell:
            r"""
uuidTmpOut=$(uuidgen)
...
rule colorBedAccordingToGffCompare:
    ...
    output:
        "output/mappings/mergedReads/colored/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.splicing_status-all.endSupport-{endSupport}.bed",
    shell:
        """
uuidTmpOut=$(uuidgen)
...
rule getGffCompareStats:
    input:
        "output/mappings/mergedReads/gffcompare/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.splicing_status-{splicedStatus}.endSupport-{endSupport}.vs.gencode.simple.tsv",
    output:
        "output/statsFiles/"
        + "tmp/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.splicing_status-{splicedStatus}.endSupport-{endSupport}.vs.gencode.stats.tsv",
    shell:
        r"""
uuidTmpOut=$(uuidgen)
...
rule getGffCompareGencodeSnPrStats:
    input:
        "output/mappings/mergedReads/gffcompare/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.splicing_status-{splicedStatus}.endSupport-{endSupport}.vs.gencode.adj.simple.tsv",
    output:
        "output/statsFiles/"
        + "tmp/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.splicing_status-{splicedStatus}.endSupport-{endSupport}.vs.gencode.SnPr.stats.tsv",
    shell:
        r"""
uuidTmpOut=$(uuidgen)
uuid=$(uuidgen)
...
rule aggGffCompareGencodeSnPrStats:
    input:
        lambda wildcards: expand(
            ...
            sizeFrac=SIZEFRACS,
            sampleRep=SAMPLEREPS,
            endSupport=wildcards.endSupport,
            minReadSupport=wildcards.minReadSupport,
            splicedStatus=wildcards.splicedStatus,
        ),
    output:
        "output/statsFiles/"
        + "all.tmerge.min{minReadSupport}reads.splicing_status-{splicedStatus}.endSupport-{endSupport}.vs.gencode.SnPr.stats.tsv",
    shell:
        r"""
uuidTmpOut=$(uuidgen)
echo -e "seqTech\tcapDesign\tsizeFrac\tsampleRep\tlevel\tmetric\tvalue" > {TMPDIR}/$uuidTmpOut
...
rule aggGffCompareStats:
    ...
    output:
        "output/statsFiles/"
        + "all.tmerge.min{minReadSupport}reads.splicing_status-{splicedStatus}.endSupport-{endSupport}.vs.gencode.stats.tsv",
    shell:
        r"""
uuidTmpOut=$(uuidgen)
...
rule getTmVsGencodeLengthStats:
    ...
    output:
        "output/statsFiles/"
        + "tmp/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.splicing_status-{splicedStatus}.endSupport-{endSupport}.vs.gencode.length.tsv",
    shell:
        r"""
uuid=$(uuidgen)
...
rule aggTmVsGencodeLengthStats:
    ...
    output:
        "output/statsFiles/"
        + "all.tmerge.min{minReadSupport}reads.endSupport-{endSupport}.vs.gencode.length.stats.tsv",
    shell:
        r"""
uuidTmpOut=$(uuidgen)
...
rule mergeTmsWithGencode:
    ...
    output:
        "output/mappings/mergedReads/gencodeMerge/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.endSupport-{endSupport}.+gencode.gff.gz",
    threads: 1
    shell:
        """
uuidTmpOut=$(uuidgen)
...
rule makeClsGencodeLoci:
    input:
        "output/mappings/mergedReads/gencodeMerge/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.endSupport-{endSupport}.+gencode.gff.gz",
    params:
        locusPrefix=config["PROJECT_NAME"],
    output:
        temp(
...
rule mergeWithRef:
    ...
    output:
        "output/mappings/mergedReads/mergeToRef/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.endSupport-{endSupport}.+gencode.loci.refmerged.gff.gz",
    shell:
        """
uuid=$(uuidgen)
uuidTmpOut=$(uuidgen)
...
rule getNovelIntergenicLociQcStats:
    input:
        tmergeGencode="output/mappings/mergedReads/mergeToRef/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.endSupport-{endSupport}.+gencode.loci.refmerged.gff.gz",
        repeats=lambda wildcards: config["REPEATMASKER_DIR"]
...
rule aggNovelIntergenicLociQcStats:
    ...
    output:
        "output/statsFiles/"
        + "all.tmerge.min{minReadSupport}reads.endSupport-{endSupport}.novelLoci.qc.stats.tsv",
    shell:
        r"""
uuid=$(uuidgen)
...
rule getNovelIntergenicLociStats:
    ...
    output:
        "output/statsFiles/"
        + "tmp/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.endSupport-{endSupport}.novelLoci.stats.tsv",
    shell:
        r"""
uuidTmpOut=$(uuidgen)
...
rule tmergeAll:
    input:
        tm=lambda wildcards: expand(
            "output/mappings/mergedReads/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.HiSS.tmerge.min{minReadSupport}reads.splicing_status-{splicedStatus}.endSupport-{endSupport}.gff.gz",
            filtered_product,
            techname=TECHNAMES,
...
rule getAnnotatedGeneDetectionStats:
    ...
    output:
        "output/statsFiles/"
        + "tmp/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.splicing_status-{splicedStatus}.endSupport-{endSupport}.gencode.geneDetection.stats.tsv",
    shell:
        r"""
uuid=$(uuidgen)
file="$(dirname {input})/$(basename {input} .simple.tsv).tmap"
...
rule aggAnnotatedGeneDetectionStats:
    ...
    output:
        "output/statsFiles/"
        + "all.min{minReadSupport}reads.splicing_status-{splicedStatus}.endSupport-{endSupport}.gencode.geneDetection.stats.tsv",
    shell:
        r"""
uuid=$(uuidgen)
echo -e "seqTech\tcapDesign\tsizeFrac\tsampleRep\tcategory\tcount\tpercent" > {TMPDIR}/$uuid
cat {input} | awk '{{print $1,$2,$3,$4,"flGenes",$6,$8"\n"$1,$2,$3,$4,"nonFlGenes",$7,$9}}' OFS="\t" | sort -T {TMPDIR} >> {TMPDIR}/$uuid
mv {TMPDIR}/$uuid {output}
        """
...
rule aggNtCoverageByGenomePartition:
    ...
    output:
        "output/statsFiles/"
        + "all.tmerge.min{minReadSupport}reads.endSupport-{endSupport}.vs.ntCoverageByGenomePartition.stats.tsv",
    shell:
        r"""
uuid=$(uuidgen)
