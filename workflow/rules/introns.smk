rule makeIntrons:
    ...
    output:
        "output/mappings/makeIntrons/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.introns.gff.gz",
    shell:
        """
uuidTmpOut=$(uuidgen)
...
zcat {input.introns} | {{ grep -vP "^ERCC" || [ "$?" -eq 1 ]; }} | extract_intron_strand_motif.pl - {input.genome} {TMPDIR}/$uuid/$(basename {output.gff} .introns.gff.gz)
...
rule getGencodeSpliceJunctions:
    input:
        lambda wildcards: GENOMETOANNOTGTF[CAPDESIGNTOGENOME[wildcards.capDesign]],
    output:
...
rule getClsSpliceJunctions:
    input:
        "output/mappings/mergedReads/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.HiSS.tmerge.min{minReadSupport}reads.splicing_status-spliced.endSupport-all.gff.gz",
    output:
        "output/mappings/mergedReads/spliceJunctions/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.spliceJunctions.list",
    shell:
        """
uuidTmpOut=$(uuidgen)
...
rule getCompareClsGencodeSJsStats:
    ...
    output:
        "output/statsFiles/"
        + "tmp/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.vs.Gencode.SJs.stats.tsv",
    shell:
        r"""
uuidTmpOut=$(uuidgen)
...
rule aggCompareClsGencodeSJsStats:
    ...
    output:
        "output/statsFiles/"
        + "all.tmerge.min{minReadSupport}reads.vs.Gencode.SJs.stats.tsv",
    shell:
        r"""
uuidTmpOut=$(uuidgen)
