...
rule removePolyAERCCs:
    ...
    output:
        temp(
            "output/mappings/removePolyAERCCs/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.polyAsitesNoErcc.tmp.bed"
        ),
    shell:
        """
uuidTmpOut=$(uuidgen)
...
rule getPolyAreadsList:
    input:
        "output/mappings/removePolyAERCCs/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.polyAsitesNoErcc.bed",
    output:
        "output/mappings/getPolyAreadsList/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.polyAreads.list",
    shell:
        """
uuidTmpOut=$(uuidgen)
...
rule aggPolyAreadsStats:
    ...
    output:
        "output/statsFiles/" + "all.polyAreads.stats.tsv",
    shell:
        r"""
uuidTmpOut=$(uuidgen)
