...
rule longReadMapping:
    input:
        reads=lambda wildcards: expand(
            ...
            sizeFrac=wildcards.sizeFrac,
            sampleRep=wildcards.sampleRep,
        ),
...
rule getBamqcStats:
    ...
    output:
        "output/statsFiles/"
        + "tmp/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.sequencingError.stats.tsv",
    shell:
        """
uuid=$(uuidgen)
...
rule aggBamqcStats:
    ...
    output:
        "output/statsFiles/" + "all.sequencingError.stats.tsv",
    shell:
        """
uuid=$(uuidgen)
...
rule makeBigWigExonicRegions:
    input:
        ...
        annotGff=lambda wildcards: GENOMETOANNOTGTF[
            CAPDESIGNTOGENOME[wildcards.capDesign]
        ],
    conda:
        "../envs/xtools_env.yml"
...
rule setupReadProfileMatrix:
    ...
    output:
        colorList="output/statsFiles/"
        + "byTech_{capDesign}_{sizeFrac}_{sampleRep}.colors.txt",
...
cat << 'R_SCRIPT' > {output.colorList}.r
library(tidyverse);
...
rule plotReadProfileMatrix:
    input:
        ...
        + "byTech_{capDesign}_{sizeFrac}_{sampleRep}.colors.txt",
    output:
        profile="output/plots/"
        + "readProfile/byTech_{capDesign}_{sizeFrac}_{sampleRep}.readProfile.density.png",
...
rule aggMappingStats:
    ...
    output:
        "output/statsFiles/" + "all.basic.mapping.stats.tsv",
    shell:
        """
uuidTmpOut=$(uuidgen)
...
rule aggSpikeInsMappingStats:
    ...
    output:
        "output/statsFiles/" + "all.spikeIns.mapping.stats.tsv",
    shell:
        r"""
uuidTmpOut=$(uuidgen)
...
rule readBedToGff:
    input:
        "output/mappings/readBamToBed/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.bed.gz",
    output:
        "output/mappings/readBedToGff/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.gff.gz",
    shell:
        """
uuidTmpOut=$(uuidgen)
...
rule getReadBiotypeClassification:
    ...
    params:
        grepSpikeIns=lambda wildcards: (
            ...
            if wildcards.spikeInCategories == "woSpikeIns"
            else ""
        ),
    shell:
        r"""
...
rule getReadToBiotypeBreakdownStats:
    ...
    output:
        "output/statsFiles/"
        + "tmp/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.readToBiotypeBreakdown.{spikeInCategories}.stats.tsv",
    shell:
        r"""
...
rule aggReadToBiotypeBreakdownStats:
    input:
        lambda wildcards: expand(
            ...
            sizeFrac=SIZEFRACS,
            sampleRep=SAMPLEREPS,
            spikeInCategories=wildcards.spikeInCategories,
        ),
    output:
        "output/statsFiles/"
        + "all.readToBiotypeBreakdown.{spikeInCategories}.stats.tsv",
    shell:
        r"""
uuidTmpOut=$(uuidgen)
...
rule plotReadToBiotypeBreakdownStats:
    ...
    params:
        ...
        BtPalette=lambda wildcards: (
            simpleBiotypes_Rpalette_woSpikeins
            if wildcards.spikeInCategories == "woSpikeIns"
            else simpleBiotypes_Rpalette
        ),
    shell:
        r"""
cat << 'R_SCRIPT' | R --slave
